...
const HomePage = () => {
    ...
    useEffect(() => {
        async function getEmployeeDet() {
            const response = await axios.get("http://localhost:4000/GetEmployeeDetails");
    ...
    const createEmployee = async (e) => {
        ...
        try {
            const d = new Date();
            if (name.current.value != "" && email.current.value != "" && mobile.current.value != "" && designation.current.value != "" && gender.current.value != "" && course.current.value != "" && f_Image != "") {
                const data = {
                    f_Id: Math.random(),
                    f_Image,
                ...
                const response = await axios.post("http://localhost:4000/CreateEmployee", data);
    ...
    return (
        <div>
            <div className="d-flex justify-content-between p-3" style={{ backgroundColor: "rgb(222,234,246)", border: "1px solid black" }}>
                <a style={{ cursor: "pointer" }} onClick={() => setDisplayContent("DashBoard")}>Home</a>
                <a style={{ cursor: "pointer" }} onClick={() => setDisplayContent("Create Employee")}>Create Employee</a>
                <a>Employee Edit</a>
                <a style={{ cursor: "pointer" }} onClick={() => setDisplayContent("Employee List")}>Employee List</a>
                <div>
                    <a>{localStorage.getItem("userName")} - </a>
                    <a style={{ cursor: "pointer" }} onClick={() => navigate("/")}>Logout</a>
                </div>
            </div>
            <p style={{ backgroundColor: "yellow", fontSize: "25px" }}>{displayContent}</p>

            {displayContent === "DashBoard" && <h4 className="w-auto" style={{ marginLeft: "540px", marginTop: "200px" }}>Welcome To Admin Panel</h4>}

            {displayContent === "Create Employee" && <div className="w-50 ms-5 mt-5">
                <form onSubmit={createEmployee}>
                    ...
                    <div className="row w-100 mb-3">
                        ...
                        <div className="col">
                            <input type="text" className="form-control" id="Email" ref={email} onChange={(e) => validateEmail(e)}></input>
                    ...
                    <div className="row w-100 mb-3">
                        ...
                        <div className="col">
                            {f_Image != "" && <img src={f_Image} width={50} className="mb-3"></img>}
            ...
            {displayContent === "Employee Edit" && <div className="w-50 ms-5 mt-5">
            ...
            {displayContent === "Employee List" && <div className="container">
